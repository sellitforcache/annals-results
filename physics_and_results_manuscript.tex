...
CPU/GPU hardware, why chosen - workstation and cluster versions of cpu and gpu, make "history power" historys/s per purchase cost for each unit (whether that is card or node or workstation) and histories/s per watt. 

table:

Bk node
some high performing workstation (mine?)
m270
k20
titan
FOR EACH:  price, memory, processors, threads, frequencies, 

run at maximum number of histories per batch as possible for GPUs 
run CPUs at maximum number of MPI processes and OMP threads in the station or node, but with identical batch parameters as the GPU runs
